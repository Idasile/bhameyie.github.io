...
In the [previous article]({% post_url 2017-04-27-microservices-isolation-patterns %}), we introduced a problem for us to solve using different isolation patterns for Microservices. The first one we'll consider is the traditional one: keeping the data separate but communicating of HTTP/RPC.
...
Based on the response we get, we could then pass on the info to some Karma Granter service that responsible for karma re-computation.
...
Let's say that, for whatever reason, the Karma Granter Service is down. Since such an outage can occur, perhaps you will add some retry logic for your requests. If the outage takes more than a few seconds however, your comment service would have become unusable, sending back errors on every requests.
